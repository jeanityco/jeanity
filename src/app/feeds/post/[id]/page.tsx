...
export default function PostCommentsPage() {
  ...
  const { posts, getPost, getComments, loadCommentsForPost, addComment } = useFeedsPosts();
  ...
  useEffect(() => {
    ...
    void loadCommentsForPost(id).finally(() => setCommentsLoading(false));
  }, [id, loadCommentsForPost]);
  ...
  const displayPost = useMemo(() => {
    ...
    return { ...post, comments: Math.max(post.comments, comments.length) };
      ...
      <div className={shellMainColumn}>
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-1 gap-6 px-3 py-5 sm:gap-8 sm:px-6 sm:py-6 md:px-8 md:py-8 xl:gap-10">
          <div className="flex min-w-0 flex-1 flex-col gap-6 lg:gap-8">
            <FeedsPostCard post={displayPost} postIndex={postIndex} mode="detail" />
